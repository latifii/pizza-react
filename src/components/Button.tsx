...
import { ButtonProps, Varient } from "../types/button.type";
const base =
...
const Button: React.FC<ButtonProps> = ({
  ...
  const styles: Record<Varient, string> = {
  ...
  if (to) {
    return (
      <Link to={to} className={styles[varient]}>
        {children}
      </Link>
    );
  }
  if (onClick) {
    return (
      <button onClick={onClick} disabled={disabled} className={styles[varient]}>
        {children}
      </button>
    );
  }
  return (
    <button disabled={disabled} className={styles[varient]}>
      {children}
    </button>
  );
};
